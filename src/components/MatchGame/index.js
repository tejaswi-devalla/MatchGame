...
class MatchGame extends Component {
  ...
  generateRandImg = () => {
    const tot = imagesList[Math.floor(Math.random() * imagesList.length)].id
  ...
  getImage = () => {
    ...
    const finalImg = imagesList.find(
  ...
  filterAccordingToActiveTab = () => {
    const {activeTabId} = this.state
    const images = imagesList.filter(
  ...
  render() {
    const {activeTabId, score, timer, isGameFinished} = this.state
    const filteredImages = this.filterAccordingToActiveTab()
    const randomImage = this.getImage().imageUrl
    return (
      <div className="background-img">
        <NavBar score={score} timer={timer} />
        {isGameFinished ? (
          <div className="gameOverBgImg">
            <img
              src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
              alt="trophy"
              className="trophy-icon"
            />
            <p className="text-1">Your Score</p>
            <h1 className="text-2">{score}</h1>
            <button
              type="button"
          ...
          <>
            <img src={randomImage} alt="match" className="match-img" />
            <ul className="tab-list">
              {tabsList.map(eachItem => (
                <TabList
                  ...
                  isTabActive={activeTabId === eachItem.tabId}
                />
              ))}
            </ul>
